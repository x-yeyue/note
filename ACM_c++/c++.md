...
### 一、一句话定义
`std::list` 是 STL 提供的**双向链表容器**，不支持随机访问，但在**插入、删除、合并**操作上具有极致的 O(1) 效率。
...
| **随机访问** | ✅ O(1) (支持 `[]`) | ❌ 不支持 |
| **中间插入/删除** | ❌ O(n) (需移动后续元素) | ✅ O(1) (只需改指针) |
| **合并两个容器** | ❌ O(k) (需复制元素) | ✅ O(1) (使用 `splice`) |
| **迭代器失效** | 插入/删除可能导致失效 | 插入/删除/合并均不失效 |

---

### 三、核心特性
1.  **双向迭代**：可以从前往后或从后往前遍历。
2.  **节点独立**：每个元素存储在独立节点中，包含数据和前后指针。
3.  **不连续存储**：内存地址不连续，无法通过偏移量直接访问。

---

### 四、灵魂函数：`splice()` (剪切)
这是 `list` 独有的、最强大的函数，用于**将一个链表的节点移动到另一个链表中**。

#### 1. 作用
将源链表 (`src`) 的节点**剪切**（Move）到目标链表 (`dest`) 的指定位置。
- **不复制数据**，只修改内部指针。
- **时间复杂度 O(1)**。
- 源链表 (`src`) 被剪切后会变空。
...
#### 2. 常用语法
```cpp
// 1. 将 src 的全部内容剪切到 dest 的 pos 位置之前
dest.splice(pos, src);

// 2. 将 src 中 it 指向的单个元素剪切到 dest 的 pos 位置之前
dest.splice(pos, src, it);

// 3. 将 src 中 [first, last) 区间的元素剪切到 dest 的 pos 位置之前
...
int main() {
    ...
    // 1. 插入
    ls.push_back(1);   // 尾部插入 O(1)
    ls.push_front(2);  // 头部插入 O(1)
    auto it = ls.begin();
    ls.insert(++it, 3); // 在指定位置插入 O(1)

    // 2. 删除
    ls.pop_back();     // 尾部删除 O(1)
    ls.pop_front();    // 头部删除 O(1)
    ls.erase(ls.begin()); // 删除指定迭代器 O(1)

    // 3. 遍历 (只能用迭代器或范围 for)
    for (int x : ls) { /* ... */ }

    // 4. 自带排序 (list 不能用 std::sort，必须用自带的)
    ls.sort();
...
### 六、适用场景
当遇到以下情况时，优先考虑使用 `std::list`：
1.  **需要频繁在容器中间进行插入或删除**。
2.  **需要频繁合并两个容器**（使用 `splice` 秒杀）。
3.  **需要保证迭代器在插入/删除后不失效**。
