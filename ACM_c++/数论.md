...
> 给定两个正整数 $a$ 和 $b$，$a$ 和 $b$ 的最大公约数($greatest \; common \; dividor$， 或称最大公因子)定义为能够同时整除 $a$ 和 $b$ 的最大正整数，记为 $gcd(a,b)$，有 
> $$gcd\left(a, b\right) = \max\left\{k, k\mid a \text{ 且 } k \mid b\right\}$$
